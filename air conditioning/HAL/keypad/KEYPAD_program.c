...
void KEYPAD_init(void)
{
  ...
  DIO_setpindir(DIO_PORTC, COL_1, DIO_PIN_OUTPUT);
  DIO_setpindir(DIO_PORTC, COL_2, DIO_PIN_OUTPUT);
  DIO_setpindir(DIO_PORTC, COL_3, DIO_PIN_OUTPUT);
...
uint8_t KEYPAD_getpressedkey(void)
{
  ...
  for (uint8_t counter_col = 0; counter_col < KEYPADSIZE; counter_col++)
  {
	  ...
	  DIO_setpinvalue(DIO_PORTC, Col_Arr[counter_col], DIO_PIN_HIGH);
  }
...
for (uint8_t counter_col = 0; counter_col < KEYPADSIZE; counter_col++)
  {
    ...
    DIO_setpinvalue(DIO_PORTC, Col_Arr[counter_col], DIO_PIN_LOW);
     ...
     for (uint8_t counter_row = 0; counter_row < KEYPADSIZE; counter_row++)
    {
      ...
      DIO_readpin(DIO_PORTA, Row_Arr[counter_row], &colValue);
      if (colValue == 0)
      {
        // Return pressed button value
        DIO_setpinvalue(DIO_PORTA, Row_Arr[counter_row], DIO_PIN_HIGH);
